...
[![License](https://img.shields.io/github/license/Cohexa-ai/agent-coherence-plugin)](LICENSE)
...
| `agent-coherence-migrate-deny` *(v0.8.0+)* | Stricter sibling: STDOUT-only, symlink-contained, never invokes an LLM, never writes settings.json | Security-sensitive workspaces; CI-driven migration where auto-apply is not acceptable |
...
The marketplace add resolves to the latest published release. To pin a specific version: `/plugin marketplace add Cohexa-ai/agent-coherence-plugin@v0.2.0`.
...
> strict mode requires `agent-coherence>=0.8.0` once published; warn-mode (the
...
For tool-class restrictions (`grep` → `rg`, no `python -c`, no `sudo`), the structurally stronger primitive is `permissions.deny` at the configuration layer. `agent-coherence-migrate-deny` (v0.8.0+) is the security-hardened helper: STDOUT-only (never writes settings.json), symlink-contained (canonical-path containment check refuses files outside the workspace root), never invokes an LLM. Operator reviews the output and pastes into `.claude/settings.local.json`.
...
| `strict_mode.yaml` *(v0.8.0+)* | Per-artifact strict-mode opt-in. Same shape. Same intersection semantics with `tracked_paths`. | Operator's choice |
| `audit.log` *(v0.8.0+)* | Append-only JSONL of strict-mode denial events. Mode `0600`. Denials only — no command bodies, no user content. | ✓ |
