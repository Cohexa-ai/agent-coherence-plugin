...
- **The release preflight now fails on a branch ruleset that forbids the documented release merge.** `main` is guarded by two independent systems — classic branch protection and the `protect-main` ruleset — which stack and are evaluated separately, and `tools/check_release_readiness.js` only ever inspected the first (plus the *tag* ruleset, a different object with different rules). The second drifted unseen into `allowed_merge_methods: ["squash"]` with `required_linear_history`, directly contradicting `docs/RELEASE.md` §2 step 2, which merges the release with a merge commit. v0.4.0 paid for it: the release was squash-merged, `main` was left with no ancestry link to `dev`, and the next forward-merge came back as add/add conflicts across every file the release touched, resolved by hand in [#112](https://github.com/Cohexa-ai/agent-coherence-plugin/pull/112). Preflight reported **0 failures** the whole time. A new check asserts that any active branch ruleset covering the default branch omits `required_linear_history`, includes `merge` in `allowed_merge_methods` (an absent list means all methods are permitted, and is accepted), and carries an admin bypass actor — without which `main`'s structurally unsatisfiable review requirement makes every release unmergeable. The rule logic is a pure exported function (`evaluateBranchRuleset`) so `src/test/release_readiness_branch_ruleset.test.ts` exercises it with no network round-trip, pinning the exact v0.4.0 shape as a failing case.

### Fixed

- **A preempted session's coordinator notice now says what Python says, and only reaches sessions that actually lost a write grant.** Two divergences from the Python coordinator, which is the reference implementation for this prose, had been live on both backends' `dev` — masked in the cross-implementation protocol corpus by a fixture that ignored `additionalContext` wholesale. First, `registry.acquireExclusive` queued a pending notice for *every* invalidated peer, `commit` and `commitCas` included, so preempting a SHARED **reader** told it `⚠ Your EXCLUSIVE grant on this artifact was silently revoked` — a grant it never held, stacked on top of the stale-read warning it already gets, while the same scenario on Python produced no notice at all. Recording is now restricted to victims that held MODIFIED or EXCLUSIVE, matching Python's `_peers_in_me_excluding` snapshot; SHARED peers are still invalidated, they just aren't told they lost something. Second, the prose itself was a different message: no per-line warning that the stranded edit **lands in your worktree but is NOT reflected in the coordinator's version**, no closing re-read guidance, no cap — so a session returning to a long-preempted workspace got an unbounded list against Claude Code's 10KB `additionalContext` budget. `preemptionNoticeText` now renders Python `_build_preemption_text` byte for byte: newest first, three notices verbatim, the remainder coalesced into a `Plus N more` line pointing at `/agent-coherence status`, and the ADV-004 sweep sentinel named as a coordinator reclamation rather than printed as if its first eight hex digits were a session id. Notice timestamps also carry sub-second precision now (they were truncated to whole seconds, which both dropped same-second re-preemptions at the `ON CONFLICT` guard and wrote an integer into a column Python writes a float into). Pinned on both sides: `src/test/preemption_notice.test.ts` covers the plural, overflow and sweep branches, and three protocol-corpus fixtures byte-assert the wire prose against Python and Node together.
- **A Node major upgrade no longer kills the coordinator permanently.** Provisioning was keyed on a `diff` of `package.json`, which detects a *package* change but is blind to a *Node ABI* change — and native-module validity depends on `process.versions.modules`, which `package.json` cannot express. Provision under Node 23 (ABI 131), install Node 24 (ABI 137), and every session from then on crash-looped `The module '…/better_sqlite3.node' was compiled against a different Node.js version using NODE_MODULE_VERSION 131. This version of Node.js requires NODE_MODULE_VERSION 137.` with nothing to break the cycle. A package bump did not rescue it either: the 0.3.1 → 0.4.0 diff *did* fire, but `npm install` saw `better-sqlite3` already present at a satisfying version and skipped it, leaving the ABI-131 binary exactly where it was. `bin/ensure-coordinator-node` now stamps the provisioning ABI to `${CLAUDE_PLUGIN_DATA}/.node-abi` and treats a mismatch as an install trigger alongside the `package.json` diff, **tearing down `node_modules` before installing** so `prebuild-install` refetches the prebuilt for the running ABI (`npm rebuild` is deliberately not used — it compiles from source via node-gyp, the Python dependency decision B forbids). Trees provisioned before the stamp existed are probed rather than reinstalled, so upgrading costs a healthy install nothing and needs no network; and a `NODE_MODULE_VERSION` death at the post-spawn liveness gate invalidates the stamp, turning any remaining case of this failure into one bad session instead of a permanent one.
