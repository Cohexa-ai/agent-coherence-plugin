...
- **The release preflight now fails when `dev`'s lockfile is behind `main`'s.** Dependabot builds its dependency graph from the **default branch only** — `.github/dependabot.yml` says so in its own comment — so a security advisory produces exactly one alert and one PR, both scoped to `main`, and `dev` is never examined even though every feature branch is cut from it. That makes the `main` → `dev` forward-merge half of the remediation rather than release hygiene, and it is the half that gets skipped. The record is unambiguous: alert #5 (js-yaml, 2026-08-23) records `fixed_at` **four seconds** after PR [#102](https://github.com/Cohexa-ai/agent-coherence-plugin/pull/102) merged to `main` and **forty-one minutes before** PR [#109](https://github.com/Cohexa-ai/agent-coherence-plugin/pull/109) carried the same bump to `dev` — the alert reported green while `dev` was still running the vulnerable version. By the time alert #6 opened on the same package, twelve main-only commits had accumulated since that last forward-merge, every one authored by `dependabot[bot]`. Nothing in preflight looked at this, because every other check inspects branch protection or workflow shape and none compared the two branches' resolved dependency versions. `tools/check_release_readiness.js` gained `evaluateLockfileDrift`, a pure verdict function (so `src/test/release_readiness_lockfile_drift.test.ts` exercises the rule logic with no network round-trip) plus a read-only wrapper that reads both branches' `package-lock.json`. The comparison is keyed on `name@major` **lineage**, taking each side's **lowest** version within a lineage. Not by lockfile path, because npm hoists the same package between `node_modules/x` and `node_modules/dep/node_modules/x`, so a path key misses `dev` holding a vulnerable copy nested under a dependency. Not by bare **name** either: one name is routinely installed at two lineages at once — this repo has `ignore` at 5.3.2 and 7.0.5, and `eslint-visitor-keys` at 3.4.3 and 5.0.1 across four paths — which makes both branches collapse to the same shared minimum and hides a bump to the higher lineage entirely. Lowest-within-lineage is what still catches a branch carrying both the patched and the vulnerable copy. A `main` lineage with no `dev` counterpart falls back to `dev`'s highest lower major, so a bump that crosses a major boundary is caught rather than skipped: of the twenty packages the check found on its first live run, three (`file-entry-cache` 8→11, `flat-cache` 4→6, `keyv` 4→5) were major bumps, and a lineage key without that fallback would have reported seventeen and called the rest clean. A lineage `dev` carries and `main` does not is `dev`'s own dependency tree, not a missed forward-merge, and is deliberately not flagged. Only `dev`-older-than-`main` is a finding: `dev` leading is the normal state, a package absent from `dev` is a different question, npm aliases resolve through their `name` field so two branches aliasing one specifier to different packages are never compared, and versions that are not plainly comparable (prerelease, git ref) are skipped rather than guessed. An unreadable **or structurally empty** lockfile **fails** rather than warning — a guard that cannot obtain its evidence must not certify, and an object-shaped `{"packages":{}}` would otherwise compare nothing and reach the certifying PASS text. HTTP 403 is the one exception, since that means the token cannot read contents at all rather than that anything is wrong with the lockfiles. When every drifted copy is nested under one of `dev`'s own dependencies, the verdict drops the forward-merge instruction and says so, because merging `main` into `dev` cannot change `dev`'s transitive tree and would strand the operator at a gate they could not clear. What the drift costs is contributor and CI exposure: `dev`, and every feature branch cut from it, installs and tests the older version. It does **not** silently regress `main` at release time — a three-way merge keeps `main`'s side of a line `dev` never touched, which was verified against a real merge rather than assumed. On its first live run the check found 19 drifted packages.
